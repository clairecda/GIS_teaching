# QGIS Onboarding Guide

This guide helps learners install QGIS, verify the setup, and prepare a project workspace that aligns with the course structure.

## 1. Choose the right QGIS build
...
- Download links:
  ...
  - **Windows:** Standalone installer (64-bit) — prefer the LTR option.
  - **Linux:** Use the distribution-specific repositories listed on the QGIS site; stick with the LTR channel.
- Avoid installing from unofficial package managers (e.g., Homebrew, OSGeo4W) unless you are comfortable troubleshooting.
...
3. In the main window:
   ...
   - Switch to **Dark Theme** or high-contrast theme if preferred (Settings ▶ Options ▶ General).
...
- Course Slack `#qgis-help` for quick questions.
- Office hours (listed in syllabus) for screen-share troubleshooting.
- QGIS documentation: [https://docs.qgis.org/](https://docs.qgis.org/) — keep the LTR docs bookmarked.
